"""UMAP precomputation for Strong's embeddings."""
...
EXPECTED_STRONGS_COUNT = 14298
TABLE_NAME = 'strongs_embeddings'
UMAP_TABLE_NAME = 'umap_strongs_coords'
...
def check_existing_umap_results(conn: psycopg2.extensions.connection) -> bool:
    ...
    try:
        with conn.cursor() as cur:
            ...
            return count == EXPECTED_STRONGS_COUNT
    except Exception as e:
        print(f"Warning: Error checking existing UMAP results: {e}")
        return False


def extract_embeddings(conn: psycopg2.extensions.connection) -> List[Tuple[int, np.ndarray]]:
    """Extract strongs_id and parsed embeddings from database."""
    print("Extracting Strong's embeddings from database...")
    embeddings = []
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT strongs_id, embedding FROM {TABLE_NAME} WHERE model_name = %s ORDER BY strongs_id",
                (MODEL_NAME,)
            )
            rows = cur.fetchall()
            print(f"Extracted {len(rows)} embeddings for model '{MODEL_NAME}'")
            for strongs_id, embedding_text in rows:
                try:
                    # Parse pgvector string format: "[1.0,2.0,...]" -> remove brackets and split
                    vec = np.fromstring(embedding_text[1:-1], sep=',')
                    embeddings.append((strongs_id, vec))
                except Exception as e:
                    print(f"Failed to parse embedding for strongs_id {strongs_id}: {e}")
        return embeddings
    except Exception as e:
...
def run_umap_reduction(vectors: np.ndarray) -> np.ndarray:
...
def insert_umap_results(conn: psycopg2.extensions.connection, data: List[Tuple]):
    ...
    try:
        with conn.cursor() as cur:
            ...
            sql = f"""
                INSERT INTO {UMAP_TABLE_NAME} (strongs_id, x, y, z, model_name)
                VALUES %s
...
def main():
    ...
    try:
        ...
        if check_existing_umap_results(conn):
            print(f"UMAP results for '{MODEL_NAME}' already exist with {EXPECTED_STRONGS_COUNT} entries. Skipping computation.")
            return

        print(f"\n{'='*70}")
        print(f"UMAP Precomputation for Strong's Embeddings")
        print(f"{'='*70}\n")

        # Step 1: Extract embeddings
        embeddings = extract_embeddings(conn)
        if len(embeddings) != EXPECTED_STRONGS_COUNT:
            print(f"Warning: Expected {EXPECTED_STRONGS_COUNT} entries, but got {len(embeddings)}")

        if not embeddings:
            print("No embeddings extracted. Aborting.")
            sys.exit(1)

        # Separate strongs_ids and vectors
        strongs_ids, vectors = zip(*embeddings)
        ...
        upsert_data = [
            (strongs_id, float(x), float(y), float(z), MODEL_NAME)
            for strongs_id, (x, y, z) in zip(strongs_ids, coords)
        ]
